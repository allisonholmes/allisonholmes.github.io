...
Cleaning and re-coding surveillance data for monthly report to CDC.
...
# reference https://www.r-bloggers.com/2019/09/create-rmarkdown-document-with-sas-code-and-output-sas-engine/

```

``` {r sas_setup, include = FALSE}
...
*** USE SERVER: SASApp_name ***;

libname M odbc noprompt="SERVER=server_name;DRIVER=SQL Server Native Client 11.0;Trusted_Connection=YES; 
DATABASE=db_name" schema=DBO;

libname mav '\\pathname\SAS_Data'; 
run;
...
Filtering to year-end cases for 2021
...
/* OPTION 2: Pulling specific cases - for XDR requests */
/*
	data events; set events_check;
...
### Recode variables to CDC format
	
```{r CDC_recode, eval = FALSE}
...
libname CDC '\\pathname\Electronic Files sent to CDC\2023';
...
OUTFILE= "\\pathname\Electronic Files sent to CDC\2023\NYC_TyphiCases.xls"
DBMS=EXCEL REPLACE; SHEET="NYC_TyphiCases"; /*edit path*/
RUN;

```

### Check that cases are marked as transmitted

```{r status_check, eval = FALSE}

/***AFTER SUBMISSION TO CDC -
...
/*CHECK that all cases reported in last data pull have been updated*/
	* join to .SAS file of last pull;
proc sql;
	SELECT stateepi AS event_id, REPORT_TO_CDC, REPORT_TO_CDC_DATE 
	FROM CDC.NYC_TyphiCases AS report
